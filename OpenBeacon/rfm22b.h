...
inline void narcoleptic_calibrate() {
  ...
  // Capture Timer 1 state
   uint8_t TCCR1Acopy;   TCCR1Acopy = TCCR1A;
...
*/
	...
	spiWriteRegister(0x1c, 0xc1); //  IF Filter Bandwidth
	n=4;
    }else{
	spiWriteRegister(0x1c, 0xd1);
	n=5;
    }
    ...
    register unsigned long ncoff = (p.CallToneFreq << (n+16)) / 11719;
    
    spiWriteRegister(0x20, (byte)rxosr);
    spiWriteRegister(0x21, (byte)( ((rxosr & 0x300) >>2) | ((ncoff >> 16) & 0x0f ) ) );

    spiWriteRegister(0x22, (byte)(ncoff >> 8));
    spiWriteRegister(0x23, (byte)ncoff); //  clock recovery offset 0


    uint16_t k3=(rxosr >> 15) + 2;
